...
getUserProfil() {
  this.role = localStorage.getItem('role'); // 👈 rôle stocké au login (apprenant, formateur, admin)
  const userId = localStorage.getItem('userId');

  if (this.role === 'APPRENANT') {
    this.apprenantService.getApprenantByid(userId).subscribe(res => {
      this.userProfil = {
        username: res.lastName + " " + res.firstName,
        photo: res.photo || 'assets/images/avatar-apprenant.png'
      };
    });
  }
  else if (this.role === 'FORMATEUR') {
    this.formateurService.getFormateurByid(userId).subscribe(res => {
      this.userProfil = {
        name: res.username,
        photo: res.photo || 'assets/images/avatar-formateur.png'
      };
    });
  }
